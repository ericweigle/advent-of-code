...
def part2(paths, data):
  ...
  visited = [0]
  memoized = dict()
  explore(data, paths, 0, nonzero_flows, location=1, time_left=30, memoized=memoized, visited=visited, pressure=0)
  print(f"Memoized: {len(memoized)}, Visited: {visited}")
  print("Best in 26 seconds:", max(memoized.values()))

  pressure = max(memoized.values())
  print(f"Part1: Relieved {pressure} pressure")
